...
---
자동 생성되는 것 (Schema): 모델(models.py)을 정의하고 makemigrations -> migrate를 하면 테이블과 기본 권한(Permission) 목록(add, change, delete, view)은 자동으로 생성됩니다.
자동 생성되지 않는 것 (Data): 그룹(Group) 자체나, 어떤 그룹이 어떤 권한을 갖는지(Mapping) 에 대한 정보는 "데이터" 영역이기 때문에 자동으로 만들어지지 않습니다. 개발자가 직접 넣어줘야 합니다.

> **Data Initialization**: `python manage.py init_test_data` 명령어를 실행하면 `Faker` 라이브러리를 사용하여 현실적인 테스트 데이터(환자, 의사, 진료기록 등)가 자동으로 생성됩니다. 자세한 내용은 `DB_INIT_GUIDE.md`를 참고하세요.

> **Note**: 현재 프로젝트는 기본적으로 **로컬 MySQL** (`neuronova` DB)을 사용하도록 설정되어 있습니다. Docker 환경에서는 `host.docker.internal`을 통해 호스트 DB에 연결합니다.
...
| **auth_group**                 | 역할 그룹(Role)      | ❌ 사용 안 함 (UserProfile.role 사용) |
| **auth_group_permissions**     | 그룹 ↔ 권한 매핑       | ❌ 사용 안 함    |
| **auth_user_user_permissions** | 유저 개별 권한 추가      | ❌ 사용 안 함   |
